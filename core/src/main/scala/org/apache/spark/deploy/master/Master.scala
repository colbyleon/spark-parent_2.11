...
private[deploy] class Master(
  ...
  override def receive: PartialFunction[Any, Unit] = {
    ...
    case DriverStateChanged(driverId, state, exception) =>
      state match {
          // 如果driver的状态是错误、完成、杀死、失败则移除driver
  ...
  private def scheduleExecutorsOnWorkers(
    ...
    def canLaunchExecutor(pos: Int): Boolean = {
      // 是否还需要分配 min（App需要的core,所有worker剩余的core数量）
      val keepScheduling = coresToAssign >= minCoresPerExecutor
      // 当前worker的核数大于minCore
    ...
    while (freeWorkers.nonEmpty) {
      freeWorkers.foreach { pos =>
        ...
        while (keepScheduling && canLaunchExecutor(pos)) {
          ...
          // 不设置core数时，默认一个worker上只启动一个executor
          // 这里设置为1和+1仅为了canLaunchExecutor()做判断
          ...
          // 如果是spreadOut则在当前worker上分配一次最小core后，轮到下一个worker继续分配，否则直到榨干当前worker为止
          if (spreadOutApps) {
  ...
  private def startExecutorsOnWorkers(): Unit = {
    ...
    // 首先，遍历waitingApps中的AppInfo，并且过滤出还需要core的AppInfo
  ...
  private def schedule(): Unit = {
    ...
    // 因为standalone和yarn-client模式都会在本地直接启动driver,而不会来注册driver,更不可能调度driver了
    // 找到内存和core数量满足要求的work为止
    // 下一个driver从最后一次启动了driver的worker之后一个开始
  ...
  private def registerApplication(app: ApplicationInfo): Unit = {
    ...
    // 这里，其实是将app的信息加入内存缓存中
    apps += app
    idToApp(app.id) = app
    endpointToApp(app.driver) = app
    addressToApp(appAddress) = app

    // 将app加入等待调度的ArrayBuffer
    waitingApps += app
    if (reverseProxy) {
